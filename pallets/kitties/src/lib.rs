...
pub mod pallet {
	...
	#[pallet::type_value]
	pub fn GetDefaultValue() -> KittyIndex { // 定义结构的默认值获取方式
		0_u32
	}

	#[derive(Encode, Clone, Debug, PartialEq, Eq, Decode, TypeInfo, MaxEncodedLen)]
	pub struct Kitty(pub [u8; 16]); // 为每个kitty 属性分配存储空间， 16个u8 数据存储来保存它
	...
	impl<T: Config> Pallet<T> {
		...
		fn random_value(sender: &T::AccountId) -> [u8; 16] {
			let payload = (
				...
				&sender,
				<frame_system::Pallet::<T>>::extrinsic_index(), // 类似nonce
			);
